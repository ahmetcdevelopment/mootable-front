"use client";

import { useState } from "react";
import { Button } from "@/shared/ui/Button";
import { ChromeIcon, GithubIcon, Loader2Icon } from "lucide-react";

interface OAuth2Provider {
  name: string;
  enabled: boolean;
  loginUrl: string;
}

interface OAuth2LoginButtonsProps {
  className?: string;
}

export function OAuth2LoginButtons({ className }: OAuth2LoginButtonsProps) {
  const [loading, setLoading] = useState<string | null>(null);

  const handleOAuth2Login = (provider: string) => {
    setLoading(provider);
    // Redirect to backend OAuth2 endpoint
    window.location.href = `${process.env.NEXT_PUBLIC_API_URL}/oauth2/login/${provider.toLowerCase()}`;
  };

  return (
    <div className={className}>
      <div className="relative">
        <div className="absolute inset-0 flex items-center">
          <span className="w-full border-t border-ink-800" />
        </div>
        <div className="relative flex justify-center text-xs uppercase">
          <span className="bg-ink-950 px-2 text-ink-400">Or continue with</span>
        </div>
      </div>

      <div className="mt-6 grid grid-cols-2 gap-3">
        {/* Google Login Button */}
        <Button
          type="button"
          variant="secondary"
          disabled={loading !== null}
          onClick={() => handleOAuth2Login("Google")}
          className="relative"
        >
          {loading === "Google" ? (
            <Loader2Icon className="mr-2 h-4 w-4 animate-spin" />
          ) : (
            <GoogleIcon className="mr-2 h-4 w-4" />
          )}
          Google
        </Button>

        {/* Microsoft Login Button */}
        <Button
          type="button"
          variant="secondary"
          disabled={loading !== null}
          onClick={() => handleOAuth2Login("Microsoft")}
          className="relative"
        >
          {loading === "Microsoft" ? (
            <Loader2Icon className="mr-2 h-4 w-4 animate-spin" />
          ) : (
            <MicrosoftIcon className="mr-2 h-4 w-4" />
          )}
          Microsoft
        </Button>
      </div>

      <p className="mt-4 text-center text-xs text-ink-400">
        By signing in with social accounts, you agree to follow the white rabbit
        and accept our terms of service.
      </p>
    </div>
  );
}

// Custom Google Icon
function GoogleIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none">
      <path
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
        fill="#4285F4"
      />
      <path
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
        fill="#34A853"
      />
      <path
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
        fill="#FBBC05"
      />
      <path
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
        fill="#EA4335"
      />
    </svg>
  );
}

// Custom Microsoft Icon
function MicrosoftIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none">
      <path fill="#F25022" d="M11.4 11.4H2V2h9.4v9.4z" />
      <path fill="#7FBA00" d="M22 11.4h-9.4V2H22v9.4z" />
      <path fill="#00A4EF" d="M11.4 22H2v-9.4h9.4V22z" />
      <path fill="#FFB900" d="M22 22h-9.4v-9.4H22V22z" />
    </svg>
  );
}